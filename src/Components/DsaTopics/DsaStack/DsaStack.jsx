...
import { dsaStackQuestions } from "../../../data/DsaData/stackQuestion.js";
import {
  Box,
  Card,
  CardContent,
...
const DsaStack = () => {
  ...
  const uniqueDays = useMemo(
    () =>
      Array.from(new Set(dsaStackQuestions.map((q) => q.day))).sort(
        (a, b) => a - b
      ),
    []
  );
  ...
  const uniqueDifficulties = useMemo(
    () => Array.from(new Set(dsaStackQuestions.map((q) => q.difficulty))),
    []
  );

  const filteredQuestions = useMemo(
    () =>
      dsaStackQuestions.filter((q) => {
    ...
    >
      ...
      >
        <Box textAlign="center" mb={3}>
          ...
          >
            📚 DSA Stack — Interview Essentials
          </Typography>
          <Typography
            variant="subtitle1"
            ...
            sx={{ mt: 1 }}
          >
            Cover MinStack, expression eval, monotonic stacks, histogram & more.
          </Typography>
        </Box>
