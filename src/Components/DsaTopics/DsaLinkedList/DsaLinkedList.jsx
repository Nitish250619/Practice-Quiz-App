...
import { dsaLinkedListQuestions } from "../../../data/DsaData/linkedlistQuestion.js";
import {
  Box,
  Card,
  CardContent,
...
const DsaLinkedList = () => {
  ...
  const uniqueDays = useMemo(
    () =>
      Array.from(new Set(dsaLinkedListQuestions.map((q) => q.day))).sort(
        (a, b) => a - b
      ),
    []
  );
  ...
  const uniqueDifficulties = useMemo(
    () => Array.from(new Set(dsaLinkedListQuestions.map((q) => q.difficulty))),
    []
  );

  const filteredQuestions = useMemo(() => {
    return dsaLinkedListQuestions.filter((q) => {
    ...
    >
      ...
      >
        <Box textAlign="center" mb={3}>
          ...
          >
            🔗 DSA Linked List — Core Interview Sheet
          </Typography>
          <Typography
            variant="subtitle1"
            ...
            sx={{ mt: 1 }}
          >
            Practice reversal, cycles, merges, K-groups, random pointers & more.
          </Typography>
        </Box>
