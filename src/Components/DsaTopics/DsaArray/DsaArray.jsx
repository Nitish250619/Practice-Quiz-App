...
import { dsaArrayQuestions } from "../../../data/DsaData/arraysQuestion.js";
import {
  Box,
  Card,
  CardContent,
...
const DsaArray = () => {
  ...
  // Unique days and difficulties from data (for flexibility)
  const uniqueDays = useMemo(
    () => Array.from(new Set(dsaArrayQuestions.map((q) => q.day))).sort(
      (a, b) => a - b
    ),
    []
  );
  ...
  const uniqueDifficulties = useMemo(
    () => Array.from(new Set(dsaArrayQuestions.map((q) => q.difficulty))),
    []
  );

  const filteredQuestions = useMemo(() => {
    return dsaArrayQuestions.filter((q) => {
      const matchDay = dayFilter === "all" || q.day === Number(dayFilter);
      const matchDifficulty =
        difficultyFilter === "all" || q.difficulty === difficultyFilter;

      const text = `${q.ques} ${q.pattern} ${q.topicGroup} ${q.tags?.join(" ") ?? ""}`.toLowerCase();
      const matchSearch =
        searchTerm.trim() === "" ||
    ...
    >
      ...
      >
        ...
        <Box textAlign="center" mb={3}>
          ...
          >
            📚 DSA Arrays — Must Solve Questions
          </Typography>
          <Typography
            variant="subtitle1"
            ...
            sx={{ mt: 1 }}
          >
            Filter by day, difficulty or search a pattern to focus your practice.
          </Typography>
        </Box>
              ...
              >
                <CardContent sx={{ pb: 2 }}>
                  ...
                  >
                    ...
                    >
                      {openSolution === q.id ? "Hide Solution" : "View Solution"}
                    </Button>
                  </Box>
