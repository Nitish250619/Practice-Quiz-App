...
import { dsaRecursionQuestions } from "../../../data/DsaData/recursionQuestion.js";
import {
  Box,
  Card,
  CardContent,
...
const DsaRecursion = () => {
  ...
  const uniqueDays = useMemo(
    () =>
      Array.from(new Set(dsaRecursionQuestions.map((q) => q.day))).sort(
        (a, b) => a - b
      ),
    []
  );
  ...
  const uniqueDifficulties = useMemo(
    () => Array.from(new Set(dsaRecursionQuestions.map((q) => q.difficulty))),
    []
  );

  const filteredQuestions = useMemo(
    () =>
      dsaRecursionQuestions.filter((q) => {
      ...
      >
        <Box textAlign="center" mb={3}>
          ...
          <Typography
            ...
            sx={{ mt: 1 }}
          >
            From basics to N-Queens, DP-on-trees and hardcore backtracking.
          </Typography>
        </Box>
